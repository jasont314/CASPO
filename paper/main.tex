...
\paragraph{Contributions.} We contribute (i) \caspo{}: a method that drops
\vineppo{}'s prefix MC step in favor of an \ipvrm{}-style prefix value model,
including the step-TD construction and an online update path with ADB and DLW
corrections; (ii) a unified comparison stack that implements \ppo{},
\grpo{}, \vineppo{}, and \caspo{} under one trainer, one verifier, and one
rollout engine, controlling everything except the algorithmic credit-signal
choice; (iii) a CASPO advantage-transform ablation that probes the role of the
prefix-value parameterization; (iv) a frozen-RM \caspo{} ablation that turns
off the online value update; and (v) implementation infrastructure for
full-finetune 7B-scale RL with FSDP-sharded trainer and rank-local vLLM
generation, including a CUDA-IPC weight-sync path that is collective with
\texttt{summon\_full\_params}.
...
\subsection{Advantage-transform ablation}
\label{sec:transforms}

A subtle modeling choice in \caspo{} is \emph{which} statistic of $V_\phi$
should enter the step-TD difference. The three natural choices arise from
how the IPVRM loss \eqref{eq:ipvrmloss} parameterizes correctness. Recall
that the BCE margin loss treats $V_\phi$ as a Bradley--Terry score: the
implied per-prefix correctness probability is
\begin{equation}
\label{eq:ptilde}
...
Given this, the step-TD advantage in \eqref{eq:adv} can be computed on three
different scales:
...
\paragraph{When each variant should help.}
If $V_\phi$ is well-calibrated as a Bradley--Terry score, \emph{prob} is the
mathematically natural choice and admits a clean expected-reward
interpretation. If $V_\phi$ is well-trained but operates over a wide
log-ratio range that the BCE objective does not directly bound, \emph{value}
preserves more of the underlying signal but pays in variance. \emph{logprob}
is a middle ground: it preserves linear sensitivity in the negative tail
(where most gradient signal lives early in training) while compressing the
positive tail (where over-confident prefixes would otherwise dominate the
batch). Section~\ref{sec:results} reports the three variants on MATH-500
pass@$16$ to determine which scale matches the empirical
calibration of $V_\phi$ best for our setup.
...
advantages over $V_\phi$.
\end{description}
We additionally report two \caspo{} ablations: \emph{advantage-transform
prob/logprob} and \emph{frozen-RM} (no online value updates).
...
sample efficiency.
\item \caspo{} advantage-transform ablation (\emph{value} vs.\ \emph{prob} vs.\
\emph{logprob}) on MATH-500 pass@$16$.
\item \caspo{} frozen-RM ablation (online value updates on/off).
...
correctness). The frozen-RM ablation isolates this risk.
