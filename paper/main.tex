...
Equation~\eqref{eq:adv} computes the TD difference on the raw cumulative
log-ratio $V_\phi$. Two ablations are included:
\begin{align}
\text{prob:}     \quad &
A(s_t) = r_t + \gamma\, \sigma(V_\phi(s_{t+1})) - \sigma(V_\phi(s_t)), \\
\text{logprob:}  \quad &
A(s_t) = r_t + \gamma\, \log \sigma(V_\phi(s_{t+1})) - \log \sigma(V_\phi(s_t)).
\end{align}
The terminal verifier reward enters identically in all three variants.
