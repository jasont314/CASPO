...
and one set of hyperparameters, and report wall-clock and accuracy on the
MATH benchmark for both Rho-1B and DeepSeekMath-7B. \ppocritic{} is the
...
\begin{table}[t]
\centering
\caption{Shared training hyperparameters (Rho-1B and DeepSeekMath-7B). All
fields are matched to the upstream \vineppo{} configuration for the
respective scale; the only deliberate difference is the model. Values
reflect the paper-faithful alignment after the upstream-divergence audit
...
\midrule
Group size $G$ & 8 & 8 \\
Prompts per outer step & 64 & 64 \\
Responses per outer step & 512 & 512 \\
...
KL coefficient & $10^{-2}$ ($k_3$) & $10^{-2}$ ($k_3$) \\
Advantage clip & $3.0$ & $3.0$ \\
Advantage standardization & batch & batch \\
Rollout sampling & temp $0.6$, top-$p$ $0.9$ & temp $0.6$, top-$p$ $0.9$ \\
Max prompt / response & 1024 / 1024 & 1512 / 1024 \\
Max sequence length & 2048 & 2499 \\
Warmup steps & 480 & 480 \\
Outer steps & 1000 & 1000 \\
Checkpoint cadence & every 100 (steps 100--1000) & every 200 (steps 200--1000) \\
\bottomrule
\end{tabular}
\end{table}

Both configurations land at 512 responses per outer step ($64 \times G{=}8$),
2 PPO epochs per rollout, and a 64-response global PPO minibatch, matching
the upstream \vineppo{} hyperparameters at each scale
\citep{vineppo2024,vineppocode}. Apart from the base policy, every entry in
Table~\ref{tab:hp} is identical between the two scales, isolating any
empirical difference to the model rather than to the optimization recipe.
...
methods under identical conditions on Rho-1B and DeepSeekMath-7B. Empirical results
